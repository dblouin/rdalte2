...
public class RequirementTreeLabelProvider extends TreeCellLabelProvider {
	...
	private final ILabelProvider labelProvider = new LabelProvider() {
		...
		public String getText( final Object p_object ) {
			...
			if ( p_object instanceof DesignElementReference ) {
				return getText( ( (DesignElementReference) p_object ).getDesignElement() );
			}
		...
		public Image getImage( final Object p_object ) {
			...
			if ( p_object instanceof DesignElementReference ) {
				return getImage( ( (DesignElementReference) p_object ).getDesignElement() );
			}
	...
	protected Image getColumnImage(	final Object p_element,
		...
		switch ( pi_columnIndex ) {
			...
			case 1:
	        	if ( p_element instanceof VerifiableDesignElementRef ) {
	        		return verifiedImage( ( (VerifiableDesignElementRef) p_element ).getVerified() );
	        	}
	        	else if ( p_element instanceof Requirement ) {
	        		return verifiedImage( ( (Requirement) p_element ).getVerified() ); 
	        	}
	        	else if ( p_element instanceof RequirementsPackage ) {
	        		return verifiedImage( ( (RequirementsPackage) p_element ).getVerified() ); 
	        	}
			...
			switch ( pi_columnIndex ){
		        ...
		        case 2:
		        	...
		        	if ( p_element instanceof Requirement ) {
		        		...
		        		value = ( (RequirementsPackage) p_element ).getSatisfactionLevel();
		        	}
		        	else if ( p_element instanceof Specification ) {
